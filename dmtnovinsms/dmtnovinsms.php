...
class DMTNovinSMS extends Module
{
    ...
    public function install()
    {
        ...
        if (!parent::install() OR !$this->registerHook('newOrder') OR !$this->registerHook('updateOrderStatus')  OR !$this->registerHook('createAccount'))
            return false;
    ...
    protected function getPhoneMobiles($getTotal=false)
    {
        ...
        if(!$getTotal)
        {
            $query='SELECT `phone_mobile` FROM `'._DB_PREFIX_.'address`';
        ...
        {
            $sql='SELECT COUNT(*) FROM `'._DB_PREFIX_.'address` WHERE `phone_mobile` !=""';
